...
import logo from '../assets/images/logo_ys_dark.png'
...
const NavbarDark = () => {
  ...
  return (
    <nav className='w-full px-[4vw] absolute z-50 top-0 py-5 flex justify-between items-center gap-5'>
        <a href="/"><img src={logo} alt="YS Logo" className='lg:w-44 w-32 opacity-100 hover:opacity-70 transition-all text-dark' /></a>
        <div className="lg:flex justify-end items-center gap-10 relative hidden">
            <a href="/#teachers-training" className='text-sm font-bold text-dark hover:text-primary transition-all leading-none pb-0 -mb-1'>TEACHER TRAININGS</a>
            <a href="/discovery-call" className='text-sm font-bold text-dark hover:text-primary transition-all leading-none pb-0 -mb-1'>DISCOVERY CALL</a>
            <a href="/#graduatedHome" className='text-sm font-bold text-dark hover:text-primary transition-all leading-none pb-0 -mb-1'>TESTIMONIALS</a>
            <a href="https://yogasoulmcr.co.uk/" target='_blank' className='text-sm font-bold text-dark hover:text-primary transition-all leading-none pb-0 -mb-1'>ABOUT US</a>
            <a href="https://www.instagram.com/yogasoulmcr/" target='_blank' className='group'><FaInstagram className='fill-dark group-hover:fill-primary transition-all' size={20} /></a>
            <p onMouseEnter={() => {
            ...
            }} id="menuApply" className='bg-white shadow-md hidden lg:hidden flex-col justify-start items-start p-3 absolute top-10 right-0 origin-top-right gap-0'>
              {
                data.map((item, index) => (
        ...
        <button onClick={() => handleOpen()} className='flex lg:hidden justify-end items-center gap-2 text-dark'><IoMenu size={24} className='stroke-dark' />Menu</button>
